...
def _maybe_setup_ollama_model() -> None:
    """Detect local Ollama and configure the OpenAI-compatible endpoint."""
    import asyncio
...
def _is_stale_ollama_pin(model_cfg: dict) -> bool:
    """Compatibility hook for old small-Ollama migrations.

    Small local Ollama models are intentionally allowed in degraded mode, so an
    existing gemma2/phi pin must not force users back into setup.
    """
    return False


def _maybe_setup_default_backend() -> None:
    """Auto-detect the first usable backend for fresh Windows installs."""
    if os.environ.get("HERMES_MODEL"):
        return
    ...
    if len(options) == 1 or not sys.stdin.isatty():
        chosen = options[0]
    else:
        print("\n사용 가능한 LLM backend:")
